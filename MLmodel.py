...
data_training=pd.read_csv('training1.csv', names=CSV_COLUMN_NAMES)
data_validation=pd.read_csv('validation1.csv', names=CSV_COLUMN_NAMES)

data_training=data_training.iloc[np.random.permutation(len(data_training))]
data_training=data_training.reset_index(drop=True)

data_validation=data_validation.iloc[np.random.permutation(len(data_validation))]
data_validation=data_validation.reset_index(drop=True)

#training data
X=data_training.loc[:,'android.permission.ACCESS_ALL_DOWNLOADS':'android.permission.WRITE_USER_DICTIONARY']
Y=pd.get_dummies(data_training['MALWARE_LABEL'])

#test data
X_test=data_validation.loc[:,'android.permission.ACCESS_ALL_DOWNLOADS':'android.permission.WRITE_USER_DICTIONARY']
Y_test=pd.get_dummies(data_validation['MALWARE_LABEL'])
...
def multilayer_perceptron(x, weights, biases):
    ...
    out_layer = tf.nn.softmax(tf.matmul(layer_1, weights['out']) + biases['out'],name = "output")
    return out_layer
#weight and bias
...
accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))#session parameters
...
for epoch in range(training_epochs):
    ...
    train_accuracy_results.append(avg_acc)
...
for step in range(epoch):
    _, loss, cost=sess.run([train_step,cross_entropy,accuracy], feed_dict={x: x_input, y_:[t for t in y_input.as_matrix()]})
    train_loss_results.append(loss)
    train_accuracy_results.append(cost)
    if step%display_step==0 :
        print (loss, cost)
"""
print("Accuracy:", accuracy.eval({x: X_test, y: Y_test}), cost.eval({x: X_test, y: Y_test}))
...
# grabbing the default graph# grabb
